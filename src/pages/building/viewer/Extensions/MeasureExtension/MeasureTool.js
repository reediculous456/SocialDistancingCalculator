...
export function MeasureTool(viewer, options, sharedMeasureConfig, snapper) {
  ...
  // ITSC CODE
  /* eslint-enable */
  this.getMeasurementCount = function() {
    return Object.keys(_measurementsManager.measurementsList).length;
  };

  this.addPick = function(inp) {
    const json = JSON.parse(JSON.stringify(inp));
  ...
  this.addMeasurement = function(json) {
    ...
    _currentMeasurement.DatabaseID = json.DatabaseID;
    _currentMeasurement.result = json.result;
    _currentMeasurement.area = json.area;
    _currentMeasurement.closedArea = json.closedArea;

    for (const key in json.picks) {
      if (!json.picks[key])
  ...
  /* eslint-enable */
    const measurement = {
      DatabaseID: _currentMeasurement.DatabaseID,
      angle: _currentMeasurement.angle,
      area: _currentMeasurement.area,
      closedArea: _currentMeasurement.closedArea,
      distanceX: _currentMeasurement.distanceX,
      distanceXYZ: _currentMeasurement.distanceXYZ,
      distanceY: _currentMeasurement.distanceY,
      distanceZ: _currentMeasurement.distanceZ,
      id: _currentMeasurement.id,
      measurementType: _currentMeasurement.measurementType,
      picks: _currentMeasurement.picks,
      result: _currentMeasurement.result,
    };

    if (measurement.DatabaseID) {
      FreeformMarkupService.update({
        id: measurement.DatabaseID,
        markup: { attributes: measurement, deleted_on: new Date() },
      })
        .then(() => {
          toastr.success(`Successfully deleted measurement!`);
        })
        .catch(() => {
          toastr.error(`Could not delete measurement`);
        });
    }
    /* eslint-disable */
    // ITSC CODE
  ...
  this.handleSingleClick = function (event, button) {
    if (_consumeSingleClick) {
      ...
      /* eslint-enable */
        ...
        if (_measurementsManager && !_currentMeasurement.DatabaseID) {
          FreeformMarkupService.create({
            attributes: measurement,
            created_on: new Date(),
            type_id: MARKUP_TYPE_ID_AREA,
            urn_id: _options.urn.id,
          })
            .then(markup => {
              let createdMeasurement;

              for (const measurement_index in _measurementsManager.measurementsList) {
                if (_measurementsManager.measurementsList[measurement_index].id === _currentMeasurement.id) {
                  createdMeasurement = _currentMeasurement;
                }
              }
              _measurementsManager.measurementsList[createdMeasurement.id].DatabaseID = markup.id;
              toastr.success(`Succesfully created measurement!`);
            })
            .catch(() => {
              toastr.error(`Could not create measurement!`);
            });
        }
        ...
        if (_measurementsManager && _currentMeasurement.DatabaseID) {
          FreeformMarkupService.update({
            ...
            .then(() => {
              toastr.success(`Successfully updated measurement!`);
            })
            .catch(() => {
              toastr.error(`Failed to update measurement!`);
            });
        }
        /* eslint-disable */
        // ITSC CODE
